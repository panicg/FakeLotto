 ...
 * @param to 종료일
 *
 * @return diffDay 양수 -> from이 더 뒷날짜   음수 ->  from은 과거날짜
 *
 */
fun calTwoDate(from: Date, to: Date): Long {
    val diffDay = (from.time - to.time) / (24 * 60 * 60 * 1000)
    return diffDay
}

fun getToday() : Date{
    return Date().convertString("yyyy-MM-dd").convertDate()!!
}
...
fun Activity.setTopMargins(v: View) {
